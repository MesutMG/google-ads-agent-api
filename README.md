...
* *OR* **Python 3.12+** and **`uv`** (for local development without Docker)
...
To stop the service and completely wipe the container, image, and build caches from the server:
...
The service runs on port `6161`. Interactive OpenAPI documentation is available at `http://localhost:6161/docs`.

* `POST /chat`: Main OpenAI iterative agent endpoint.
* **Payload:** `{"prompt": "Ağustos ayında en çok harcama yapan kampanyalar hangileri?"}`


* `POST /execute-tool`: Direct execution of a Google Ads MCP tool.
